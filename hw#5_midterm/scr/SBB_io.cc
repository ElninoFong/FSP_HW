...
SBB_instrument_input_file::get_records(int& length) 
{
	...
	while(std::fgets(_line_buf,SBB_LINE_BUFFER_LENGTH,_file)){
		...
		// the line should be:
		// SecurityID Ticker SettlementDate CouponRate MaturityDate Frequency RateType Rate Quality Amount
		...
		// Rate (later we will load in spread and a string token of an OTR. Also will have a separate file
