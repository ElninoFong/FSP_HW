...
public:
	...
	// calculate the Term(in years) in yield curve
	static int
	get_number_of_periods_annual( SBB_date from_dt, SBB_date to_dt)
	{
		int annual_period_count = 0;
		while (from_dt != to_dt) {
			to_dt.add_months(-12);
			annual_period_count++;
		}
		return annual_period_count;
	}
};
